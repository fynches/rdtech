$(document).ready(function () {  
    ...
    oTable =  $('#staticblock_index').DataTable({
            processing: true,
            serverSide: true,
            autoWidth: false,
            bRetrieve: true,
            ...
            ajax: window.base_url + '/admin/staticblock/getData',
            ...
            fnCreatedRow: function(nRow, aData, iDataIndex) {
                ...
                $('td:eq(5)', nRow).html(''+
                    '<a href="'+ window.base_url + '/admin/staticblock/'+aData.id+'/edit"><i class="fa icon-muted fa-pencil icon-space"></i></a>');
                    //'<a href="javascript:void(0);" class="delete_record" onclick="deleteStaticBlock('+aData.id+')"><i class="fa icon-muted fa-times icon-space"></i></a>');
...
function deleteStaticBlock(id) {
        if (confirm('Are you sure you want to delete this record?')) {
            $.ajax({
                type: "GET",
                url: '/admin/staticblock/delete/' + id, //resource
                success: function (affectedRows) {
                    //if something was deleted, we redirect the user to the users page, and automatically the user that he deleted will disappear
                    if (affectedRows > 0)
                    {
                        window.location = 'staticblock';
                    }
                }
            });
        }
...
function bulk_actions()          
    {
        ...
        if(bulk_value == 1)
        {
            ...
            if(confirm_flag === true)
            {
                $.ajax({
                type: "GET",
                url: 'delete_multiple_StatickBlock/'+selected_row_array, 
                dataType: "html",
